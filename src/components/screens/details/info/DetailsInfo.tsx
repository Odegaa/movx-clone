...
const DetailsInfo: React.FC<IProps> = ({ movie, keywords }) => {
  const keywordsList = keywords?.keywords;
  const runtime = movie?.runtime ? `${Math.floor(movie?.runtime / 60) < 10 ? "0" : ""
    }${Math.floor(movie?.runtime / 60)}hr ${movie?.runtime % 60}min` : "00hr 00min";
